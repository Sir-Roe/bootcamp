...
```{r}
df <- read.csv("C:/Users/Logan/Documents/GitHub/bootcamp/week_7/day_3/winequality-white.csv")
...
```{r}
df <- read.csv("C:/Users/Logan/Documents/GitHub/bootcamp/week_7/day_3/winequality-white.csv", sep = ';')
...
df_long <- data.frame(
  id = rep(1:3, 2),
  variable = rep(c('var1', 'var2'), 3),
  year = rep(c(2010,NA),3),
  value = c(2,3,4,10,12,14)
)
df_long

df_long %>% mutate(across(everything(), ~ replace_na(., 2011)))
...
```{r}
dfc <- read.csv("C:/Users/Logan/Documents/GitHub/bootcamp/week_7/day_3/winequality-red.csv", sep = ';')

dfc <- my_fun(dfc)

dfc %>% summarize(na_count = sum(is.na(dfc)))

dfc_long <- pivot_longer(dfc, everything(), names_to = 'col', values_to = 'val')
dfc_long %>% group_by(col) %>% summarise(data_type = class(val))
```
...
# Separate function to split the column "names" into a first_name and last_name column
...
# Unite function brings together two columns into one, specifying how you want the values to be separated
new_df <- unite(df_new, full_name, first_name, last_name, sep = ' ')
...
# X - increase(noisily)
# Z - increase slowly
# Y so that it is inversely related to x, and positively related to x*z

# Set a seed to make my random code reproducible
set.seed(42)
...
```{r}
df <- read.csv("C:/Users/Logan/Documents/GitHub/bootcamp/week_5/homework_5_1.py/titanic.csv")
```

```{r}
summary(glm(Pclass~Fare, data = df))
```
