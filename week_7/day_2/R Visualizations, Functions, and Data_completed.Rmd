...
#_tmut is the suffix to get only the derived fields
```
...
mtcars_data <- select(mtcars,
                      everything())

mtcars_data_mut <- mutate(mtcars_data,
               disp_bycyl = (disp/cyl)/wt
               )

view(mtcars_data_mut)
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    #find subs strings from a1 in a2
    #if true return list sorted
    # Write the solution
    ...
    # I was bummed  this was a 6kyu and i struggled so hard here.
    ...
    # fizz buzz add multiples in array from range of input
...
#Murder , assualt, Rape are continuous and quantitative ratio data on crime types.

#even though Assault is an int, its per 100,000 should be seen as possibly non int any number. Likely this data was rounded.

#Urbanpop is a positive integer out of 100, it is discrete and quantitative data that represents % of population urban
...
```{r}
crime_mut <- mutate(crime,
                      violentcrime = Murder+Assault+Rape,
                      murder_to_assault= Murder/Assault)
view(crime_mut)
```

```{r}
#I can see a relationship between violent crime and urban pop%
#I predict we will see a positive relationship to the amount of crime to urban population density.
#but which crime is most prevelant in this relationship?
ggplot(data=crime_mut) +
  geom_point(mapping = aes(UrbanPop,violentcrime))

#there seems to be a surprising amount of crime  still in the lower population percentages
```

```{r}
ggplot(data=crime) +
  geom_point(mapping = aes(UrbanPop,Murder))
```

```{r}
ggplot(data=crime) +
  geom_point(mapping = aes(UrbanPop,Assault))
```

```{r}
ggplot(data=crime) +
  geom_point(mapping = aes(UrbanPop,Rape))

#rape per capita seems to have the strongest correlation of the 3 in terms of crime. Overall we do see a relationship to crime and population densities.
```
